...
bool AudioOutput::init(const std::string& device, Uint8 channels, Uint32 rate, Uint32 buffer) {
    ...
    _bitrate = sizeof(float);
    if (want.freq != _audiospec.freq) {
        // Delegate resampling to a child node
        _resampler  = AudioResampler::alloc(want.channels, _audiospec.freq);
    }
    if (want.channels != _audiospec.channels) {
        // Delegate channel distribution to a child node
        _distributor = AudioRedistributor::alloc(_audiospec.channels, _audiospec.freq);
    }
    if (want.format != _audiospec.format) {
        ...
        _bitbuffer = (float*)malloc(_pagesize*want.channels*sizeof(float));
        std::memset(_bitbuffer, 0, _pagesize*want.channels*sizeof(float));
        switch (_audiospec.format) {
            ...
            case AUDIO_S16MSB:
                ...
                _swapbits = ((want.format ^ _audiospec.format)  & SDL_AUDIO_MASK_ENDIAN);
                break;
            case AUDIO_U16LSB:
            case AUDIO_U16MSB:
...
Uint32 AudioOutput::poll(Uint8* stream, int len) {
    ...
    if (_converter != nullptr) {
        ...
        while (take < frames) {
            Uint32 amt = _pagesize < frames-take ? _pagesize : frames-take;
