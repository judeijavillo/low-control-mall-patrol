 ...
 */
void FadeOut::load(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state) {
 ...
 */
void FadeOut::update(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state, float dt) {
    Color4f color = target->getColor();
    float* data = (float*)state;
    float orig = data[0];
    //float curr = data[1];
 ...
 * @param state     The relevant node state
 */
void FadeIn::load(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state) {
    Color4f color = target->getColor();
    // Because of clamping, we need to carry along two values this time.
    float* data = (float*)state;
    data[0] = color.a;
 ...
 * @param dt        The elapsed time to animate.
 */
void FadeIn::update(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state, float dt) {
    Color4f color = target->getColor();
    float* data = (float*)state;
    float orig = data[0];
    //float curr = data[1];
