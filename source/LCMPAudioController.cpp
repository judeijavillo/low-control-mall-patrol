...
bool compareNodes(pair<float, std::string> o1, pair<float, std::string> o2) {
...
//            cout << "pop " << key << " at " << gameTime << "\n";
        ...
        if (gameTime == 0 || AudioEngine::get()->getState(key) != AudioEngine::State::PLAYING) {
            ...
            sort(_heap.begin(), _heap.end(), compareNodes);
        
            // Adjust sfx volume
            for (int i = 0; i < _heap.size(); i++) {
                if (AudioEngine::get()->isActive(_heap.at(i).second)) {
